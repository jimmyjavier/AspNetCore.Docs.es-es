...
## <a name="authenticate-users-connecting-to-a-signalr-hub"></a>Autenticar a los usuarios que SignalR se conectan a un centro
...
El siguiente es un ejemplo del `Startup.Configure` uso SignalR de y la autenticación ASP.net Core:
...
> [!NOTE]
> El orden en el que se registran SignalR y ASP.net Core middleware de autenticación es importante. Llame siempre `UseAuthentication` a `UseSignalR` antes de SignalR para que tenga un usuario `HttpContext`en.

::: moniker-end

### <a name="cookie-authentication"></a>Autenticación de cookies

En una aplicación basada en explorador, la autenticación de cookies permite que las credenciales de SignalR usuario existentes fluyan automáticamente a las conexiones. Cuando se usa el cliente del explorador, no se necesita ninguna configuración adicional. Si el usuario ha iniciado sesión en la aplicación, la SignalR conexión hereda automáticamente esta autenticación.
...
> [!NOTE]
> Se llama a la función de token de acceso **every** que se proporciona antes de SignalRcada solicitud HTTP realizada por. Si necesita renovar el token para mantener la conexión activa (porque puede expirar durante la conexión), hágalo desde esta función y devuelva el token actualizado.
...
En lugar `ClaimTypes.Name`de, puede usar cualquier valor de `User` (como el identificador de SID de Windows, etc.).
...
Una aplicación que autentica a los usuarios puede SignalR derivar los ID. de usuario de las notificaciones de usuario. Para especificar cómo SignalR crea los ID. de `IUserIdProvider` usuario, implementa y registra la implementación.
