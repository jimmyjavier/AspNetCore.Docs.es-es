...
* Opcional: puede quitar el elemento ** &lt; DotNetCliToolReference &gt; ** de `Microsoft.VisualStudio.Web.CodeGeneration.Tools` . Puede reemplazar esta herramienta por una versión instalada globalmente mediante la ejecución de `dotnet tool install -g dotnet-aspnet-codegenerator` .
* En 2,1, una [ Razor biblioteca de clases](xref:razor-pages/ui-class) es la solución recomendada para distribuir Razor archivos. Si la aplicación usa vistas incrustadas o se basa en la compilación en tiempo de ejecución de Razor archivos, agregue `<CopyRefAssembliesToPublishDirectory>true</CopyRefAssembliesToPublishDirectory>` a un `<PropertyGroup>` en el archivo del proyecto.
...
En el código siguiente se muestran los cambios en el código generado por la plantilla 2,1. Todos los cambios se agregan al código nuevo, excepto que se ha `UseBrowserLink` quitado:
...
ASP.NET Core 2,1 proporciona [ASP.net Core Identity ](xref:security/authentication/identity) como una [ Razor biblioteca de clases](xref:razor-pages/ui-class) (RCL).

La interfaz de usuario de 2,1 predeterminada Identity no proporciona actualmente nuevas características importantes en la versión 2,0. Reemplazar Identity por el paquete RCL es opcional. Las ventajas de reemplazar el código generado Identity por la plantilla con la versión RCL incluyen:

* Muchos archivos se mueven fuera del árbol de origen.
* Las correcciones de errores o las nuevas características de Identity se incluyen en el [metapaquete Microsoft. AspNetCore. app](xref:fundamentals/metapackage-app). Automáticamente Identity se actualiza cuando `Microsoft.AspNetCore.App` se actualiza.

Si ha realizado cambios no triviales en el código generado por la plantilla Identity :

* Las ventajas anteriores probablemente no **justifican** la conversión a la versión de RCL.
* Puede mantener el código ASP.NET Core 2,0 Identity , ya que es totalmente compatible.

Identity2,1 expone extremos con el `Identity` área. Por ejemplo, en la tabla siguiente se muestran ejemplos de Identity puntos de conexión que cambian de 2,0 a 2,1:
...
Las aplicaciones que tienen código que usa Identity y reemplazan Identity la interfaz de usuario 2,0 con la Identity biblioteca 2,1 deben tener en cuenta Identity las direcciones URL tienen `/Identity` un segmento antepuesto a los URI. Una manera de controlar los nuevos Identity extremos es configurar redirecciones, por ejemplo, de `/Account/Login` a `/Identity/Account/Login` .

### <a name="update-identity-to-version-21"></a>Actualización Identity a la versión 2,1

Las siguientes opciones están disponibles para la actualización Identity a 2,1.

* Use el Identity código de UI 2,0 sin cambios. El uso del Identity código de UI 2,0 es totalmente compatible. Este es un buen enfoque cuando se han realizado cambios significativos en el Identity código generado.
* Elimine el Identity código 2,0 y [el Identity scaffolding](xref:security/authentication/scaffold-identity) existentes en el proyecto. El proyecto usará la [ Razor biblioteca de clases](xref:razor-pages/ui-class)de [ASP.net Core Identity ](xref:security/authentication/identity) . Puede generar código e interfaz de usuario para cualquier Identity código de interfaz de usuario que haya modificado. Aplique los cambios de código al código de la interfaz de usuario con scaffolding.
...
### <a name="replace-identity-20-ui-with-the-identity-21-razor-class-library"></a>Reemplace Identity 2,0 UI por la Identity biblioteca de clases de 2,1 Razor

En esta sección se describen los pasos para reemplazar el código generado por la plantilla ASP.net Core 2,0 Identity por la [ Razor biblioteca de clases](xref:razor-pages/ui-class)de [ASP.net Core Identity ](xref:security/authentication/identity) . Los pasos siguientes son para un Razor proyecto de páginas, pero el enfoque para un proyecto de MVC es similar.
...
* [Scaffolding Identity ](xref:security/authentication/scaffold-identity) en el proyecto:
  * Seleccione los proyectos que salen del archivo * \_ layout. cshtml* .
...
* Elimine la Identity clase derivada generada por el scaffolding `IdentityDbContext` en la carpeta *areas/ Identity /Data/* .
* Eliminar *áreas/ Identity /IdentityHostingStartup.CS*.
* Actualice el archivo * \_ LoginPartial. cshtml* :
  * Mueva *pages/ \_ LoginPartial. cshtml* a *pages/Shared/ \_ LoginPartial. cshtml*.
  ...
  * Actualice el `<form />` elemento a `<form asp-area="Identity" asp-page="/Account/Logout" asp-route-returnUrl="@Url.Page("/Index", new { area = "" })" method="post" id="logoutForm" class="navbar-right">` .
  ...
  En el código siguiente se muestra el archivo * \_ LoginPartial. cshtml* actualizado:
...
## <a name="changes-to-razor-pages-projects-razor-files"></a>Cambios en Razor los archivos de proyectos de páginas Razor

### <a name="the-layout-file"></a>El archivo de diseño

* Mueva *pages/ \_ layout. cshtml* a *pages/Shared/ \_ layout. cshtml*
* En *areas/ Identity /pages/ \_ ViewStart. cshtml*, cambie `Layout = "/Pages/_Layout.cshtml"` a `Layout = "/Pages/Shared/_Layout.cshtml"` .
* El archivo * \_ layout. cshtml* tiene los siguientes cambios:
...
* *Páginas/ \_ ValidationScriptsPartial. cshtml* se mueve a *pages/Shared/ \_ ValidationScriptsPartial. cshtml*.
...
## <a name="changes-to-mvc-projects-razor-files"></a>Cambios en los archivos de proyectos de MVC Razor
...
## <a name="changes-to-the-launchsettingsjson-file"></a>Cambios en el launchSettings.jsen el archivo

Como ASP.NET Core aplicaciones ahora usan HTTPS de forma predeterminada, las *propiedades o el launchSettings.jsen* el archivo han cambiado.

El siguiente JSON muestra ellaunchSettings.jsanterior generado por la plantilla 2,0 *en* el archivo:

[!code-json[](20_21/sample/launchSettings20.json)]

El siguiente JSON muestra la 2,1 nuevalaunchSettings.jsgenerada por la plantilla *en* el archivo:
