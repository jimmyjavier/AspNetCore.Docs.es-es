...
* Cambie la plataforma de destino a .NET Core 2,1 actualizando el archivo de `<TargetFramework>netcoreapp2.1</TargetFramework>`proyecto a.
* Reemplace la referencia de paquete `Microsoft.AspNetCore.All` de por una referencia de `Microsoft.AspNetCore.App`paquete para. Es posible que deba agregar las dependencias que se quitaron de `Microsoft.AspNetCore.All`. Para obtener más información, vea <xref:fundamentals/metapackage#migrate> y <xref:fundamentals/metapackage-app>.
* Quite el atributo "version" en la referencia de paquete `Microsoft.AspNetCore.App`a. Los proyectos que `<Project Sdk="Microsoft.NET.Sdk.Web">` usan no necesitan establecer la versión. La versión de .NET Framework de destino es implícita y está seleccionada para que coincida mejor con el modo en que ASP.NET Core 2,1 funciona. Para obtener más información, vea la sección [reglas para los proyectos que tienen como destino la plataforma compartida](#rules-for-projects-targeting-the-shared-framework) .
* En el caso de las aplicaciones que tienen como destino el .NET Framework, actualice cada referencia de paquete a 2,1.
* Quite las referencias ** &lt;a&gt; ** los elementos DotNetCliToolReference de los siguientes paquetes. Estas herramientas se incluyen de forma predeterminada en el CLI de .NET Core y no es necesario instalarlas por separado.
  * Microsoft. DotNet. Watcher. Tools (`dotnet watch`)
  * Microsoft. EntityFrameworkCore. Tools. DotNet (`dotnet ef`)
  * Microsoft. Extensions. Caching. SqlConfig.`dotnet sql-cache`Tools ()
  * Microsoft. Extensions. SecretManager. Tools`dotnet user-secrets`()
* Opcional: puede quitar el `Microsoft.VisualStudio.Web.CodeGeneration.Tools` ** &lt;elemento DotNetCliToolReference&gt; ** de. Puede reemplazar esta herramienta por una versión instalada globalmente mediante la ejecución `dotnet tool install -g dotnet-aspnet-codegenerator`de.
...
* El SDK de 2.1.300 de .NET Core (primero incluido en Visual Studio 15,6) establece la versión `Microsoft.AspNetCore.App` implícita de en 2.1.0 que causó conflictos con Entity Framework Core 2.1.1. La solución recomendada es actualizar el SDK de .NET Core a 2.1.301 o posterior. Para obtener más información, consulte los [paquetes que comparten dependencias con Microsoft. AspNetCore. app no pueden hacer referencia a versiones de revisión](https://github.com/aspnet/Universe/issues/1180).
* Todos los proyectos que deben `Microsoft.AspNetCore.All` usar `Microsoft.AspNetCore.App` o deben agregar una referencia de paquete para el paquete en el archivo de proyecto, aunque contengan una referencia de proyecto `Microsoft.AspNetCore.All` a `Microsoft.AspNetCore.App`otro proyecto mediante o.

  Ejemplo:

  * `MyApp`tiene una referencia de paquete `Microsoft.AspNetCore.App`a.
  * `MyApp.Tests`tiene una referencia de proyecto `MyApp.csproj`a.

  Agregue una referencia de paquete `Microsoft.AspNetCore.App` para `MyApp.Tests`a. Para obtener más información, consulte la configuración de las [pruebas de integración es difícil de configurar y puede interrumpirse en el mantenimiento de entornos compartidos](https://github.com/dotnet/aspnetcore/issues/3156).
...
Cambie las `FROM` líneas en el *Dockerfile* para usar los nuevos nombres de imagen y las etiquetas de la columna 2,1 de la tabla anterior. Para más información, consulte [migración de aspnetcore Docker repositorios a dotnet](https://github.com/aspnet/Announcements/issues/298).
...
En el código siguiente se muestran los cambios en el código generado por la plantilla 2,1. Todos los cambios se agregan al código nuevo `UseBrowserLink` , excepto que se ha quitado:

[!code-csharp[](20_21/sample/Startup.cs?highlight=3,4,21-26,30,42,45,47)]

Los cambios de código anteriores se detallan en:

* [Compatibilidad con RGPD en ASP.net Core](xref:security/gdpr) `CookiePolicyOptions` para `UseCookiePolicy`y.
...
* [Requiere https](xref:security/enforcing-ssl#require-https) para `UseHttpsRedirection`.
...
* Las correcciones de errores o las nuevas características de la identidad se incluyen en el [metapaquete Microsoft. AspNetCore. app](xref:fundamentals/metapackage-app). La identidad actualizada se obtiene automáticamente cuando `Microsoft.AspNetCore.App` se actualiza.
...
La identidad 2,1 expone puntos de conexión con `Identity` el área. Por ejemplo, en la tabla siguiente se muestran ejemplos de puntos de conexión de identidad que cambian de 2,0 a 2,1:
...
Las aplicaciones que tienen código Identity que usa y Identity reemplazan la interfaz Identity de usuario 2,0 con la biblioteca Identity 2,1 deben `/Identity` tener en cuenta las direcciones URL tienen un segmento antepuesto a los URI. Una manera de controlar los nuevos Identity extremos es configurar redirecciones, por ejemplo, de `/Account/Login` a. `/Identity/Account/Login`

### <a name="update-identity-to-version-21"></a>Actualización Identity a la versión 2,1

Las siguientes opciones están disponibles para la Identity actualización a 2,1.

* Use el Identity código de UI 2,0 sin cambios. El Identity uso del código de UI 2,0 es totalmente compatible. Este es un buen enfoque cuando se han realizado cambios significativos en el código Identity generado.
* Elimine el Identity Código 2,0 y [el Identity scaffolding](xref:security/authentication/scaffold-identity) existentes en el proyecto. El proyecto usará la [ Razor biblioteca de clases](xref:razor-pages/ui-class)de [ASP.net Core Identity ](xref:security/authentication/identity) . Puede generar código e interfaz de usuario para cualquier código Identity de interfaz de usuario que haya modificado. Aplique los cambios de código al código de la interfaz de usuario con scaffolding.
* Elimine el Identity Código 2,0 y [scaffolding Identity ](xref:security/authentication/scaffold-identity) existentes en el proyecto con la opción de **invalidar todos los archivos**.

### <a name="replace-identity-20-ui-with-the-identity-21-razor-class-library"></a>Reemplace Identity 2,0 UI por la Identity biblioteca Razor de clases de 2,1

En esta sección se describen los pasos para reemplazar el código generado Identity por la plantilla ASP.net Core 2,0 por la biblioteca de [ Razor clases](xref:razor-pages/ui-class)de [ASP.net Core Identity ](xref:security/authentication/identity) . Los pasos siguientes son para un Razor proyecto de páginas, pero el enfoque para un proyecto de MVC es similar.
...
* Elimine las siguientes carpetas y todos los archivos que contenga:
  ...
  * *Extensiones*
* Compile el proyecto.
* [Scaffolding Identity ](xref:security/authentication/scaffold-identity) en el proyecto:
  * Seleccione los proyectos que salen del archivo * \_layout. cshtml* .
...
### <a name="update-after-scaffolding-identity"></a>Actualizar después del scaffoldingIdentity

* Elimine Identity la clase derivada `IdentityDbContext` generada por el scaffolding en la carpeta *areas/Identity/Data/* .
* Eliminar *áreas/Identity/IdentityHostingStartup.CS*.
* Actualice el * \_archivo LoginPartial. cshtml* :
  * Mueva *pages\_/LoginPartial. cshtml* a *pages/\_Shared/LoginPartial. cshtml*.
  * Agregue `asp-area="Identity"` al formulario y a los vínculos de delimitador.
  * Actualice el `<form />` elemento a `<form asp-area="Identity" asp-page="/Account/Logout" asp-route-returnUrl="@Url.Page("/Index", new { area = "" })" method="post" id="logoutForm" class="navbar-right">`.
...
## <a name="changes-to-razor-pages-projects-razor-files"></a>Cambios en Razor los archivos Razor de proyectos de páginas

### <a name="the-layout-file"></a>El archivo de diseño

* Mueva *pages\_/layout. cshtml* a *pages/\_Shared/layout. cshtml*
* En *areasIdentity/\_/pages/ViewStart. cshtml*, `Layout = "/Pages/_Layout.cshtml"` cambie `Layout = "/Pages/Shared/_Layout.cshtml"`a.
* El * \_archivo layout. cshtml* tiene los siguientes cambios:

  * `<partial name="_CookieConsentPartial" />`se ha agregado. Para más información, vea [GDPR support in ASP.NET Core](xref:security/gdpr) (Compatibilidad con el Reglamento general de protección de datos en ASP.NET Core).
  * jQuery cambia de 2.2.0 a 3.3.1.

### <a name="_validationscriptspartialcshtml"></a>\_ValidationScriptsPartial. cshtml

* *Pages\_/ValidationScriptsPartial. cshtml* se mueve a *pages\_/Shared/ValidationScriptsPartial. cshtml*.
...
## <a name="changes-to-mvc-projects-razor-files"></a>Cambios en los archivos Razor de proyectos de MVC

### <a name="the-layout-file"></a>El archivo de diseño

El archivo *layout. cshtml* tiene los siguientes cambios:

* `<partial name="_CookieConsentPartial" />`se ha agregado.
* jQuery cambia de 2.2.0 a 3.3.1
...
* El `Privacy` método de acción se agrega al controlador Home.
...
<xref:Microsoft.AspNetCore.Mvc.FileResult>ya no procesa el encabezado [Accept-Ranges](https://developer.mozilla.org/docs/Web/HTTP/Headers/Accept-Ranges) de forma predeterminada. Para habilitar el `Accept-Ranges` encabezado, establezca <xref:Microsoft.AspNetCore.Mvc.FileResult.EnableRangeProcessing> en `true`.

### <a name="controllerbasefile-and-physicalfile-range-header"></a>Encabezado de intervalo ControllerBase. File y PhysicalFile

Los siguientes <xref:Microsoft.AspNetCore.Mvc.ControllerBase> métodos ya no procesan el encabezado [Accept-Ranges](https://developer.mozilla.org/docs/Web/HTTP/Headers/Accept-Ranges) de forma predeterminada:
