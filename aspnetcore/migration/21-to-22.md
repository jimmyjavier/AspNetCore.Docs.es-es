...
Si la aplicación llama <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> a llamando `CreateDefaultBuilder` a en el [método CreateWebHostBuilder](xref:fundamentals/host/web-host#set-up-a-host) de `Program` la clase, `ConfigureKestrel` llame a para configurar el servidor Kestrel `UseKestrel` en lugar de a fin de evitar conflictos con el [modelo de hospedaje en proceso de IIS](xref:host-and-deploy/iis/index#in-process-hosting-model):
