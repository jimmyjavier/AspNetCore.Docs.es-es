...
* <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.Get*>, <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache.GetAsync*> &ndash; Acepta una clave de cadena y recupera un elemento almacenado en caché como `byte[]` una matriz, si se encuentra en la memoria caché.
...
Cuando se inicia la aplicación de <xref:Microsoft.Extensions.Caching.Distributed.IDistributedCache> ejemplo, se inserta `Startup.Configure`en. La hora actual se almacena en la <xref:Microsoft.AspNetCore.Hosting.IApplicationLifetime> memoria caché mediante (para obtener más información, vea [host de Web: interfaz IApplicationLifetime](xref:fundamentals/host/web-host#iapplicationlifetime-interface)):
